pkgs <- c("rio","readxl","jiebaR","ngram","tidyverse","parallel","taRifx","stringr","Nippon")
...
jieba_ngram <- function(data.raw=data.raw,col.analysis=col.analysis){
  ...
  fun_fth <- function(x){zen2han(as.character(x))} #FulltoHalf
  fun_bigger <- function(x){toupper(x)} #轉大寫 
  fun_space <- function(x){gsub("　",replacement="",x)}
  clusterExport(cl,"fun_fth")
  clusterExport(cl,"fun_bigger")
  clusterExport(cl,"fun_space")
  data.content <- parLapply(cl,data.content,fun_fth)
  data.content <- parLapply(cl,data.content,fun_bigger)
  data.content <- parLapply(cl,data.content,fun_space)%>% unlist()
  ...
  words_stat <- ng_count_filter
  dimension_freq <- dimension_data_freq
  dimension_doc <- dimension_data_doc
  return(words_stat)
  return(dimension_freq)
  return(dimension_doc)
}
